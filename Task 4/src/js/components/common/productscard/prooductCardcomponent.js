...
const ProductCard = (product, prefix) => {
    return `
        <div id="${prefix}-productCard-${product.id}" tabindex="0" class="col-span-6 sm:col-span-4 lg:col-span-2 border cursor-pointer border-gray-200 hover:border-(--success-dark) products-card-shadow group productCardId h-fit" aria-label="Product card">
            <div class="p-[5px] w-full h-fit relative overflow-hidden">
                ...
                <div class="absolute top-4 right-4 flex flex-col gap-1 md:gap-1.5 transform translate-x-0 md:translate-x-[100px] opacity-100 md:opacity-0 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-200 ease-in-out">
                    <button id="productAddToWishlist-${product.id}" type="button" class="m-0 p-0 size-8 md:size-10 rounded-full border border-gray-50 bg-white cursor-pointer hover:bg-gray-300 hover:border-white transition-all duration-200 ease-in-out" aria-label="Add to wishlist">
                        <i id="wishlisticon-${product.id}" class="fa-regular fa-heart text-sm md:text-xl"></i>
                    <button>
                    <button id="productmodelbtn" type="button" class="m-0 p-0 size-8 md:size-10 rounded-full border border-gray-50 bg-white cursor-pointer hover:bg-gray-300 hover:border-white transition-all duration-200 ease-in-out" aria-label="Open product model">
            ...
            <div class="p-1 md:p-3">
                <div class="flex w-full justify-between items-center ">
                    <div>
                        ...
                        ${
                            product.discount
                                ? `<span>$ ${calculateDiscountedPrice(product.baseprice, product.discount)}</span>    
                    ...
                    <button type="button" id="productAddToCart-${product.id}" class="size-6 md:size-10 flex items-center justify-center rounded-full transition-all duration-200 ease-in-out cursor-pointer bg-gray-50 group/cart hover:bg-(--light-green)" aria-label="Add product to cart"> 
                        <svg
